...
/** Profile + password management shared by the admin settings page and the member account page. */
  ...
  return (
    <Card title={t("settings.changePassword")}>
      <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className="grid gap-4">
        ...
        {/* No self-service reset: a member who forgot their current password asks an admin to reset it. */}
        <p className="text-xs text-muted-foreground">{t("settings.forgotPasswordHint")}</p>
        <div className="flex justify-end">
          <Button type="submit" disabled={mutation.isPending}>
            {mutation.isPending && <Loader2 className="h-4 w-4 animate-spin" />}
